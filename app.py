...
# 2. 頁面設定
# =========================================================
st.set_page_config(layout="wide", page_title="Cue Sheet Pro v79.6")
        ...
        for sec, sec_pct in cfg["sec_shares"].items():
            ...
            if m in ["全家廣播", "新鮮視"]:
                ...
                if cfg["is_national"]:
                    ...
                    log_entry["Details"].append(
                        ...
                        f"- 單價公式: (牌價${nat_list:,} / 標準{db['Std_Spots']}檔 * 秒數{factor} * 懲罰{total_display_penalty}) = ${nat_unit_price:,}\n"
                        f"- 總價計算: ${nat_unit_price:,} x {spots_final}檔"
                    )
                ...
                for i, r in enumerate(display_regs):
                    ...
                    if not cfg["is_national"]:
                        ...
                        log_entry["Details"].append(
                            ...
                            f"- 單價公式: (牌價${list_price_region:,} / 標準{db['Std_Spots']}檔 * 秒數{factor} * 懲罰{row_display_penalty}) = ${unit_rate_display:,}\n"
                            f"- 總價計算: ${unit_rate_display:,} x {spots_final}檔"
                        )
            ...
            # --- 家樂福 ---
                ...
                log_entry = {
                    ...
                    "Details": [
                        f"**量販總價**: ${total_rate_h:,}\n"
                        f"- 單價公式: (牌價${base_list:,} / 標準{base_std}檔 * 秒數{factor} * 懲罰{penalty}) = ${unit_rate_h:,}\n"
                        f"- 總價計算: ${unit_rate_h:,} x {spots_final}檔"
                    ]
                }
                debug_logs.append(log_entry)
