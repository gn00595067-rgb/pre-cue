...
try:
    ...
    def on_slider_change(changed_key):
        ...
        if not others: st.session_state[changed_key] = 100
            ...
            val = st.session_state[changed_key]; st.session_state[others[0]] = max(0, 100 - val)
        elif len(others) == 2:
            val = st.session_state[changed_key]; rem = max(0, 100 - val); k1, k2 = others[0], others[1]
            ...
            else: ratio = st.session_state[k1] / sum_others; st.session_state[k1] = int(rem * ratio); st.session_state[k2] = rem - st.session_state[k1]
    ...
    if is_rad:
        with m1:
            ...
            st.slider("預算 %", 0, 100, 100, key="rad_share", on_change=on_slider_change, args=("rad_share",))
            sec_shares = {}
            if len(secs) > 1:
    ...
    if is_fv:
        with m2:
            ...
            st.slider("預算 %", 0, 100, 0, key="fv_share", on_change=on_slider_change, args=("fv_share",))
            sec_shares = {}
            if len(secs) > 1:
                rem = 100; sorted_secs = sorted(secs)
                for i, s in enumerate(sorted_secs):
    ...
    if is_cf:
        with m3:
            ...
            st.slider("預算 %", 0, 100, 0, key="cf_share", on_change=on_slider_change, args=("cf_share",))
            sec_shares = {}
            if len(secs) > 1:
                rem = 100; sorted_secs = sorted(secs)
                for i, s in enumerate(sorted_secs):
    ...
    if config:
        try:
            ...
            st.error("運算發生錯誤"); st.error(traceback.format_exc())

except Exception as e:
    st.error("系統發生嚴重錯誤"); st.error(traceback.format_exc())
