...
st.set_page_config(
    layout="wide",
    page_title="Cue Sheet Pro v111.23 (Bolin Final - Refactored)"
)
...
# =========================================================
# [新增] Ragic 預設值設定
DEFAULT_RAGIC_URL = "https://ap15.ragic.com/liuskyo/cue/2" # 移除參數保留純淨路徑
...
def get_sec_factor(media_type, seconds, sec_factors):
    ...
    # Fallback logic for interpolation/estimation
...
def format_campaign_details(config):
    ...
    for media, settings in config.items():
        ...
        # 處理區域文字
        if settings.get("is_national"):
            reg_str = "全省聯播"
        else:
            reg_str = "/".join(settings.get("regions", []))
...
def load_config_from_cloud(share_url):
    try:
        ...
        if not match:
            return None, None, None, None, "連結格式錯誤"
...
def generate_excel_from_scratch(format_type, start_dt, end_dt, client_name, product_name, rows, remarks_list, final_budget_val, prod_cost, sales_person):
    ...
    def render_dongwu_optimized(ws, start_dt, end_dt, rows, budget, prod):
        ...
        for i in range(eff_days):
            ws.column_dimensions[get_column_letter(8+i)].width = 8.5
        ...
        infos = [
            ("A3", "客戶名稱：", client_name),
            ("A4", "Product：", p_str), 
            ("A5", "Period :", f"{start_dt.strftime('%Y. %m. %d')} - {end_dt.strftime('%Y. %m. %d')}"),
            ("A6", "Medium :", medium_str)
        ]
        ...
        set_border(c_spots_7, top=BS_MEDIUM, left=BS_MEDIUM)
        set_border(c_spots_8, bottom=BS_MEDIUM, left=BS_MEDIUM)
        ...
        for m_key, data in grouped_data.items():
            ...
            for r in range(start_merge, curr_row):
                ...
                set_border(ws.cell(r, spots_col_idx), left=BS_MEDIUM)
        ...
        ws.cell(sig_start, 1, "甲    方：東吳廣告股份有限公司").alignment = ALIGN_LEFT
        ...
        ws.cell(sig_start+1, right_start_col, "統一編號：").alignment = ALIGN_LEFT
        ws.merge_cells(start_row=sig_start+2, start_column=right_start_col, end_row=sig_start+2, end_column=right_start_col+7)
        ws.cell(sig_start+2, right_start_col, "客戶簽章：").alignment = ALIGN_LEFT
    ...
    def render_shenghuo_optimized(ws, start_dt, end_dt, rows, budget, prod):
        ...
        ws.merge_cells(f"A1:{get_column_letter(total_cols)}1")
        c1 = ws['A1']; c1.value = "聲活數位-媒體計劃排程表"; c1.font = Font(name=FONT_MAIN, size=24, bold=True); c1.alignment = ALIGN_CENTER
        ws.merge_cells(f"A2:{get_column_letter(total_cols)}2")
        c2 = ws['A2']; c2.value = "Media Schedule"; c2.font = Font(name=FONT_MAIN, size=18, bold=True); c2.alignment = ALIGN_CENTER
    ...
    def render_bolin_optimized(ws, start_dt, end_dt, rows, budget, prod):
        ...
        # [Manual Fix] Remove E5 right border conflict
...
def main():
    try:
        ...
        st.title("📺 媒體 Cue 表生成器 (v111.23 Bolin Final)")
