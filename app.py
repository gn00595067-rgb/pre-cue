...
# OpenPyXL Imports (Global scope to avoid import errors)
import openpyxl
...
# 1. 頁面設定
# =========================================================
st.set_page_config(layout="wide", page_title="Cue Sheet Pro v111.43 (Refactored)")
...
# =========================================================
# 7. Render Engines (Standalone Functions)
# =========================================================
# 獨立函式區：避免 Streamlit 快取閉包問題
...
def render_dongwu_optimized(ws, start_dt, end_dt, rows, budget, prod, client_name, product_name, remarks_list):
    # Dongwu logic... (Simplified for brevity, assuming standard logic)
    # Using explicit arguments
    eff_days = (end_dt - start_dt).days + 1
    spots_col_idx = 7 + eff_days + 1
    total_cols = spots_col_idx
    
    # ... (Standard Dongwu implementation reused with explicit args) ...
    # 為了節省篇幅，此處使用與前版相同的邏輯，重點是參數已正確傳遞
    ...
    ws.merge_cells(f"A1:{get_column_letter(total_cols)}1"); c = ws['A1']; c.value = "Media Schedule"; c.font = Font(name=FONT_MAIN, size=48, bold=True); c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    ...
    ws['H6'] = f"{start_dt.month}月"; ws['H6'].font = Font(name=FONT_MAIN, size=16, bold=True); ws['H6'].alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    ...
    for col, txt in headers:
        ...
        c7.border = Border(top=Side(style=BS_THIN), bottom=Side(style=BS_THIN), left=Side(style=BS_THIN), right=Side(style=BS_THIN)); c8.border = c7.border
    ...
    for i in range(eff_days):
        ...
        c_d.border = Border(top=Side(style=BS_THIN), bottom=Side(style=BS_THIN), left=Side(style=BS_THIN), right=Side(style=BS_THIN)); c_w.border = c_d.border
    ...
    c_spots_7.border = Border(top=Side(style=BS_THIN), bottom=Side(style=BS_THIN), left=Side(style=BS_THIN), right=Side(style=BS_THIN)); c_spots_8.border = c_spots_7.border
    ...
    for m_key, data in grouped_data.items():
        ...
        for idx, r in enumerate(data):
            ...
            ws.cell(curr_row, 1, display_name).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, 2, r["region"]).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, 3, r.get("program_num", 0)).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, 4, r["daypart"]).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, 5, f"{r['seconds']}秒").alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ...
            c_rate = ws.cell(curr_row, 6); c_rate.value = rate; c_rate.number_format = FMT_MONEY; c_rate.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            if pkg is not None:
                c_pkg = ws.cell(curr_row, 7); c_pkg.value = pkg; c_pkg.number_format = FMT_MONEY; c_pkg.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)

            row_sum = 0
            for d_idx in range(eff_days):
                if d_idx < len(r["schedule"]):
                    val = r["schedule"][d_idx]; row_sum += val
                    c_s = ws.cell(curr_row, 8+d_idx); c_s.value = val; c_s.number_format = FMT_NUMBER; c_s.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, spots_col_idx, row_sum).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            for c_idx in range(1, total_cols + 1):
                cell = ws.cell(curr_row, c_idx); cell.font = Font(name=FONT_MAIN, size=12); cell.border = Border(top=Side(style=BS_THIN), bottom=Side(style=BS_THIN), left=Side(style=BS_THIN), right=Side(style=BS_THIN))
            curr_row += 1
    ...
    c_lbl = ws.cell(curr_row, 5, "Total"); c_lbl.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c_lbl.font = Font(name=FONT_MAIN, size=14, bold=True)
    c_rate_sum = ws.cell(curr_row, 6, total_rate_sum); c_rate_sum.number_format = FMT_MONEY; c_rate_sum.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c_rate_sum.font = Font(name=FONT_MAIN, size=14, bold=True)
    c_val = ws.cell(curr_row, 7, budget); c_val.number_format = FMT_MONEY; c_val.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c_val.font = Font(name=FONT_MAIN, size=14, bold=True)
    total_spots_all = 0
    for d_idx in range(eff_days):
        daily_sum = sum([r['schedule'][d_idx] for r in rows if d_idx < len(r['schedule'])]); total_spots_all += daily_sum
        c = ws.cell(curr_row, 8+d_idx); c.value = daily_sum; c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c.font = Font(name=FONT_MAIN, size=12); c.number_format = FMT_NUMBER
    ws.cell(curr_row, spots_col_idx, total_spots_all).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); ws.cell(curr_row, spots_col_idx).font = Font(name=FONT_MAIN, size=12)
    ...
    for label, val in footer_items:
        ...
        ws.row_dimensions[curr_row].height = 30
        c_l = ws.cell(curr_row, 6); c_l.value = label; c_l.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True); c_l.font = Font(name=FONT_MAIN, size=12)
        c_v = ws.cell(curr_row, 7); c_v.value = val; c_v.number_format = FMT_MONEY; c_v.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c_v.font = Font(name=FONT_MAIN, size=12)
...
def render_shenghuo_optimized(ws, start_dt, end_dt, rows, budget, prod, client_name, product_name, remarks_list):
    ...
    ws.merge_cells(f"A1:{get_column_letter(total_cols)}1"); c1 = ws['A1']; c1.value = "聲活數位-媒體計劃排程表"; c1.font = Font(name=FONT_MAIN, size=24, bold=True); c1.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    ws.merge_cells(f"A2:{get_column_letter(total_cols)}2"); c2 = ws['A2']; c2.value = "Media Schedule"; c2.font = Font(name=FONT_MAIN, size=18, bold=True); c2.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    
    FONT_16 = Font(name=FONT_MAIN, size=16)
    ws.merge_cells(f"A3:{get_column_letter(total_cols)}3"); ws['A3'].value = "聲活數位科技股份有限公司 統編 28710100"; ws['A3'].font = FONT_16; ws['A3'].alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ws.merge_cells(f"A4:{get_column_letter(total_cols)}4"); ws['A4'].value = "蔡伊閔"; ws['A4'].font = FONT_16; ws['A4'].alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ...
    c5 = ws['A5']; c5.value = info_text; c5.font = FONT_14; c5.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True) 
    ...
    for i, h in enumerate(headers):
        ...
        c = ws.cell(7, c_idx); c.value = h; c.font = Font(name=FONT_MAIN, size=14, bold=True); c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
    ...
    for m_key, s_idx, e_idx in month_groups:
        ...
        c = ws.cell(7, start_col); c.value = f"{m_key[1]}月"; c.font = Font(name=FONT_MAIN, size=14, bold=True); c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c.border = BORDER_ALL_MEDIUM
    ...
    for i in range(eff_days):
        col_idx = 6 + i
        c = ws.cell(8, col_idx); c.value = curr.day; c.font = Font(name=FONT_MAIN, size=14, bold=True); c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c.border = BORDER_ALL_MEDIUM
        if curr.weekday() >= 5: c.fill = PatternFill(start_color="FFFFCC", end_color="FFFFCC", fill_type="solid")
        curr += timedelta(days=1)
    ...
    for m_key, data in grouped_data.items():
        ...
        for idx, r in enumerate(data):
            ws.row_dimensions[curr_row].height = 40
            ws.cell(curr_row, 1, d_name).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ws.cell(curr_row, 2, r['region']).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            ...
            c_spec = ws.cell(curr_row, 5, sec_txt); c_spec.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c_spec.font = Font(name=FONT_MAIN, size=10)
            row_sum = 0
            for d_idx in range(eff_days):
                if d_idx < len(r['schedule']):
                    val = r['schedule'][d_idx]; row_sum += val
    ...
    ws.cell(curr_row, 5, "Total").alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); ws.cell(curr_row, 5).font = Font(name=FONT_MAIN, size=14, bold=True)
    for d_idx in range(eff_days):
        daily_sum = sum([r['schedule'][d_idx] for r in rows if d_idx < len(r['schedule'])])
        c = ws.cell(curr_row, 6+d_idx); c.value = daily_sum; c.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c.font = Font(name=FONT_MAIN, size=14, bold=True)
    ws.cell(curr_row, end_c_start, sum([sum(r['schedule']) for r in rows])).alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); ws.cell(curr_row, end_c_start).font = Font(name=FONT_MAIN, size=14, bold=True)
...
def render_bolin_optimized(ws, start_dt, end_dt, rows, budget, prod, client_name, product_name, remarks_list):
    ...
    c1.value = "鉑霖行動行銷-媒體計劃排程表 Mobi Media Schedule"; c1.font = Font(name=FONT_MAIN, size=28, bold=True); c1.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True) 
    ...
    c2a = ws['A2']; c2a.value = "TO："; c2a.font = Font(name=FONT_MAIN, size=20, bold=True, color="FF0000"); c2a.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ws.merge_cells(f"B2:{get_column_letter(total_cols)}2"); c2b = ws['B2']; c2b.value = client_name; c2b.font = Font(name=FONT_MAIN, size=20, bold=True, color="FF0000"); c2b.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    
    c3a = ws['A3']; c3a.value = "FROM："; c3a.font = Font(name=FONT_MAIN, size=20, bold=True); c3a.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ws.merge_cells(f"B3:{get_column_letter(total_cols)}3"); c3b = ws['B3']; c3b.value = "鉑霖行動行銷 許雅婷 TINA"; c3b.font = Font(name=FONT_MAIN, size=20, bold=True); c3b.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)

    unique_secs = sorted(list(set([r['seconds'] for r in rows]))); sec_str = " ".join([f"{s}秒廣告" for s in unique_secs])
    period_str = f"執行期間：{start_dt.strftime('%Y.%m.%d')} - {end_dt.strftime('%Y.%m.%d')}"
    
    c4a = ws['A4']; c4a.value = "客戶名稱："; c4a.font = Font(name=FONT_MAIN, size=14, bold=True); c4a.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ws.merge_cells("B4:E4"); c4b = ws['B4']; c4b.value = client_name; c4b.font = Font(name=FONT_MAIN, size=14, bold=True); c4b.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
    ...
    for m_key, s_idx, e_idx in month_groups:
        ...
        c = ws.cell(5, start_col); c.value = f"{m_key[1]}月"; c.font = Font(name=FONT_MAIN, size=14, bold=True); c.alignment = Alignment(horizontal='left', vertical='center', wrap_text=True) 
    ...
    for i in range(eff_days):
        col_idx = 6 + i
        c6 = ws.cell(header_start_row, col_idx); c6.value = curr.day; c6.font = Font(name=FONT_MAIN, size=14, bold=True); c6.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True); c6.border = BORDER_ALL_MEDIUM
    ...
    for rm in remarks_list:
        ...
        c = ws.cell(current_remark_row, 6); c.value = rm; c.font = Font(name=FONT_MAIN, size=16, color=color)
...
def generate_excel_from_scratch(format_type, start_dt, end_dt, client_name, product_name, rows, remarks_list, final_budget_val, prod_cost):
    ...
    if format_type == "Dongwu": render_dongwu_optimized(ws, start_dt, end_dt, rows, final_budget_val, prod_cost, client_name, product_name, remarks_list)
    elif format_type == "Shenghuo": render_shenghuo_optimized(ws, start_dt, end_dt, rows, final_budget_val, prod_cost, client_name, product_name, remarks_list)
    else: render_bolin_optimized(ws, start_dt, end_dt, rows, final_budget_val, prod_cost, client_name, product_name, remarks_list)
...
def main():
    try:
        ...
        st.title("📺 媒體 Cue 表生成器 (v111.43 Refactored)")
